<?php
	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		$itemName = htmlentities($_POST['itemItemName']);
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				$itemSql = 'INSERT INTO item(itemNumber, itemName, discount, stock, unitPrice, status, description) VALUES(:itemNumber, :itemName, :discount, :stock, :unitPrice, :status, :description)';
				$insertItemStatement = $conn->prepare($itemSql);
