	...
	if(isset($_POST['customerDetailsCustomerID'])) {
		...
		if(isset($customerFullName) && isset($customerMobile) && isset($customerAddress)) {
			...
			if($custStatement->rowCount() > 0) {
				...
				$custSql = 'UPDATE customer SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, address2 = :address2, city = :city, district = :district, status = :status WHERE customerID = :customerID';
				$updateCustStatement = $conn->prepare($custSql);
				...
				$editCustInSale = 'UPDATE sale SET customerName = :customerName WHERE customerID = :customerID';
				$updateCustInSaleStatement = $conn->prepare($editCustInSale);
				$updateCustInSaleStatement->execute(['customerName' => $customerFullName, 'customerID' => $customerID]);
